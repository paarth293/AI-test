...
LEARNING_RATE        = 0.005
EPOCHS               = 600
CONFIDENCE_THRESHOLD = 0.60 #bug -> there was 0.95
...
def preprocess(text: str) -> list:
    ...
    return [lemmatizer.lemmatize(tok) for tok in tokens if tok.isalpha()]   #bug -> there were no digits we want vocab
...
class VanillaRNN:
    ...
    def backward(self, probs: np.ndarray, target_idx: int) -> float:
        ...
        for t in reversed(range(n)):
            dtanh  = (1.0 - self._hs[t + 1] ** 2) * d_h   # tanh derivative -> bug wrong fuction/formula was written
            d_bh  += dtanh
...
def _softmax(x: np.ndarray) -> np.ndarray:
    e_x = np.exp(x - np.max(x))  #bug -> there was no np.max(x) 
    return e_x / e_x.sum()
